        ...
        public string ChartBeatQuery(int lineid=1,int mins=30)
        {
           ...
           "sum( if (stationstate = '运行',1, 0))  AS run_c," +
            " sum( if (stationstate = '空闲', endtime - startTime, 0))/1000 AS free_t," +
             ...
             " left join ( select  stationNAME,stationstate     from huabao.LocationState where "+
             "  starttime > " + begintime + " and id_usage = " + usage_id + " and endtime is null  group by stationNAME)b1" +
